...
if CHAT_ID_RYU:
    USERS["류우"] = {
        ...
        "gender": "male",
        "trend_key": "ryu",
        "goal": "트레일 대회 대비 — 지구력·언덕(오르막/하강)·롱런 중심으로 빌드업, 부상 없이 완주/기록 단축",
...
if CHAT_ID_EK and CHAT_ID_EK != "eunkyeong_chat_id":
    USERS["은경"] = {
        ...
        "gender": "female",
        "trend_key": "eunkyeong",
        "goal": "달리기 속도(스피드) 향상 — 인터벌·템포 같은 빠른 자극으로 페이스 끌어올리기 (트레일 대회도 대비)",
...
[★ 이 사람의 훈련 목표 — 모든 운동 추천을 이 목표에 맞출 것]
{goal if goal else "체력 유지"}
...
[리포트 형식 — 정확히 이 구조. 폰으로 읽으니 짧고 스캔되게. 각 줄은 짧게]
⚠️ 텔레그램 마크다운 규칙: 굵게는 **별표 하나** `*이렇게*` (별표 둘 `**X**` 금지). 표(table)·| 기호 절대 금지(깨짐). 목록은 이모지·•·숫자로.
...
1. 운동: ★ **[훈련 목표]에 맞는 종류를 우선 처방**(목표가 속도면 인터벌/템포 우선, 트레일이면 언덕/지구력). 단순 유지·살살만 금지. **주간 흐름상 오늘 위치 고려**(매일 강할 순 없음 — 강한 날 다음은 회복). [훈련 페이스존]에서 골라 구체적 페이스(범위)+시간/거리. 회복상태에 맞춰:
...
(4) *📅 이번주 훈련 플랜* ← [훈련 목표]에 맞춰 **이번주 요일별 플랜**을 짜고 오늘({weekday_kr}요일) 위치를 표시
- 목표·다가오는 대회·최근 운동량 종합. 강-약 배치(매일 강하게 X, 강한 날 뒤엔 회복). 대회 임박이면 볼륨 줄이는 테이퍼 반영
- ⛔ **표(table)·| 기호 쓰지 말 것 (텔레그램에서 깨짐).** 요일별로 한 줄씩, 이모지 불릿으로:
  맨 위에 "목표: ○○" 한 줄, 그 아래 요일별 한 줄씩 (예: "월: 휴식 / 화: 인터벌 6×400m / 수(오늘): 이지 30분 ...")
  오늘 요일은 👉 로 강조. 마지막에 오늘 한마디.
- 오늘 추천(위)과 일관되게. 간결하게

(5) *🔬 최근 운동 분석*  ← [최근 운동 상세] 데이터 있을 때만. 가장 최근 운동을 코치처럼 자세히:
- 구간별 페이스로 **페이스 운용** 평가 (후반 무너졌나 ↘ vs 네거티브 스플릿 ↗)
- 심박(평균/최대)이 강도에 맞았나(이지인데 높으면 과함), 훈련효과(유산소/무산소)·파워가 [훈련 목표]에 부합했나
- **러닝폼**(데이터 있을 때, 눈에 띄는 1~2개만 쉬운 말로): 케이던스 170~180 권장 / 수직비율 낮을수록 효율↑(6%대 좋음·8.7%+ 개선필요) / 접지시간 짧을수록 효율 / 상하진동 낮을수록 에너지 안 새어나감
- 고도(트레일이면 의미)
- **잘한 점 1 + 개선점 1** 구체적으로. 전문용어는 꼭 쉽게 풀어서(예: "수직비율 8.8% = 위아래로 좀 많이 튀어서 힘 낭비"). 데이터 없으면 섹션 생략. 3~4문장.
